...
ビルド要件: Node.js 22.12+ / pnpm

```sh
pnpm install
pnpm run build   # dist/bundle.js を生成
```
...
1. スタンドアロンの Apps Script プロジェクトを作成し、`dist/bundle.js` を貼り付けます。
2. ライブラリとしてデプロイし、スクリプト ID を控えます。
3. 利用側プロジェクトの「ライブラリ」に識別子 `GASlacker` で追加します。
4. `GASlacker.methods(token)` で利用します。

**B. プロジェクトに直接貼り付け**

`dist/bundle.js` を自分のプロジェクトにスクリプトファイルとして追加し、
`methods(token)` を直接呼びます(`GASlacker.` プレフィックス不要)。

**C. clasp で push**

`.clasp.json` の `scriptId` を自分のものに設定して `pnpm run deploy`。
